...
合并优先级：默认值 ← 环境变量 ← 配置文件(data/email_settings.json)。
即：UI 显式保存的值最权威；环境变量（含 .env 自动加载）作为引导/兜底，
仅在文件对应字段为空时生效——否则项目自带 .env 会永久压制 UI 配置。
"""
...
def get_email_settings() -> Dict:
    """读取合并后的邮件配置（默认值 ← 环境变量 ← 文件；文件中已保存的字段最优先）。"""
    cfg = dict(_DEFAULTS)
