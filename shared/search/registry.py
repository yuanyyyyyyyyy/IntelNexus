...
from shared.search.modes import get_mode_categories
from shared.search.source import BaseSearchSource
...
class SearchSourceRegistry:
    ...
    def _load_user_sources(self):
        self._user_sources = []
        try:
            from src.config.sources import get_all_sources  # intel-briefing 路径
        except ImportError:
    ...
    def add_user_source(self, source_type: str, name: str, url: str,
        ...
        """新增用户源并刷新内存副本。返回是否成功。"""
        try:
            from src.config.sources import add_source  # noqa: F401  (兼容旧签名)
        except ImportError:
            logger.warning("未找到 src.config.sources.add_source，无法持久化用户源")
            return False
        # 扩展字段：直接写 custom_sources，使用内部持久化函数
        try:
            from src.config.sources import (get_all_sources, safe_write_json,
    ...
    def remove_user_source(self, source_id: str) -> bool:
        """删除用户源并刷新内存副本。"""
        try:
            from src.config.sources import remove_source
        except ImportError:
