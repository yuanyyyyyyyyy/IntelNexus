...
以下截图为IntelNexus软件实际运行界面截图，所有截图存放于 `docs/images/` 文件夹：

| Table编号 | 截图内容 | 对应章节 | 图片文件 |
|-----------|----------|----------|----------|
| Table 1 | CLI帮助命令输出 | 5.2.2节 | `01_cli_help.png` |
| Table 2 | Web界面主页面全貌 | 5.3.1节 | `01_homepage.png` |
| Table 3 | 侧边栏搜索模式选择 | 5.3.3节 | `02_sidebar.png` |
| Table 4 | AI模型下拉选择菜单 | 5.3.3节 | `04_model_select.png` |
| Table 5 | 线程数滑块设置 | 5.3.3节 | `05_threads.png` |
| Table 6 | 中英文语言切换 | 5.3.3节 | `15_language.png` |
| Table 7 | 搜索内容输入 | 5.3.4节 | `03_search_input.png` |
| Table 8 | 搜索执行中的加载状态 | 5.3.4节 | `08_searching.png` |
| Table 9 | 搜索结果数量与来源统计 | 5.3.4节 | `09_results.png` |
| Table 10 | AI报告流式生成过程 | 5.3.4节 | `10_generating.png` |
| Table 11 | 生成的完整情报报告 | 5.3.4节 | `11_report.png` |
| Table 12 | 按来源分组的搜索结果详情 | 5.3.4节 | `13_details.png` |
| Table 13 | 搜索结果分页导航 | 5.3.4节 | `14_pagination.png.png` |
| Table 14 | 报告下载区域 | 5.3.5节 | `12_download1.png` |
| Table 15 | 各格式下载按钮就绪状态 | 5.3.5节 | `12_download2.png` |
| Table 16 | 暗网搜索设置面板 | 5.4.1节 | `06_darkweb.png` |
| Table 17 | Tor运行状态检测结果 | 5.4.2节 | `07_tor_status.png` |

![Table 1：CLI帮助命令输出](./images/01_cli_help.png)
![Table 2：Web界面主页面](./images/01_homepage.png)
...
- [五、操作流程说明（操作手册）](#五操作流程说明)
  - [5.1 软件安装与启动](#51-软件安装与启动)
  - [5.2 命令行模式（CLI）操作](#52-命令行模式cli操作)
  - [5.3 Web界面操作详解](#53-web界面操作详解)
  - [5.4 暗网搜索操作](#54-暗网搜索操作)
  - [5.5 自定义AI模型管理](#55-自定义ai模型管理)
  - [5.6 操作流程总结](#56-操作流程总结)
- [六、性能指标与测试结果](#六性能指标与测试结果)
- [七、软件应用场景](#七软件应用场景)
- [附录A：环境变量配置详解](#附录a环境变量配置详解)
- [附录B：部署指南](#附录b部署指南)
- [附录C：常见问题FAQ](#附录c常见问题faq)
- [附录D：界面截图清单](#附录d界面截图清单)
...
本章以图文结合的方式，按"一步一图"格式详细介绍 IntelNexus 的全部操作流程，覆盖安装启动、命令行使用、Web界面完整操作、暗网搜索配置、自定义模型管理和报告导出等核心环节。
...
打开终端（Windows为PowerShell或CMD，macOS/Linux为Terminal），输入以下命令确认Python版本满足要求：

```bash
python --version
```

预期输出示例：`Python 3.10.x` 或更高版本。若未安装Python，请访问 [https://www.python.org/downloads/](https://www.python.org/downloads/) 下载安装 Python 3.10+ 版本。

**第2步：安装Ollama（可选，用于本地AI模型）**

若希望使用本地运行的AI模型进行情报分析，需要安装Ollama：

- **Windows**：访问 [https://ollama.com](https://ollama.com) 下载安装包并安装
- **macOS/Linux**：在终端执行 `curl -fsSL https://ollama.com/install.sh | sh`
...
将项目代码解压或克隆到本地目录，例如 `D:\IntelNexus`（Windows）或 `/home/user/IntelNexus`（Linux/macOS）。
...
执行完成后，终端会显示所有依赖包安装成功的提示信息。

**第4步：配置环境变量**

将项目根目录下的 `.env.example` 文件复制为 `.env`，使用文本编辑器打开 `.env` 文件，填入您申请到的API密钥（如使用云端模型）。若仅使用本地Ollama模型，可跳过此步骤。

---

### 5.2 命令行模式（CLI）操作

IntelNexus 提供命令行和Web界面两种使用方式。命令行模式适合自动化脚本、批量处理和服务器环境。

#### 5.2.1 基础搜索命令

在项目根目录下打开终端，执行以下命令进行首次搜索：

```bash
python main.py search -q "人工智能最新发展趋势" -s all
```
...
执行后终端显示所有可用命令和参数说明（如 Table 1 所示）。
...
```
[1/4] Refining query...        → 查询优化，自动纠错和多语言扩展
[2/4] Searching...             → 并行搜索多个数据源
[3/4] Keeping all results...   → 汇总去重所有搜索结果
[4/4] Scraping content...      → 并发抓取网页全文内容
[5/5] Generating summary...    → AI生成结构化情报报告
```

每步执行完成后终端输出对应的进度提示，最终报告保存为 `report_YYYY-MM-DD_HH-MM-SS.md` 文件。
...
Web界面是IntelNexus的主要交互方式，提供直观的图形化操作体验。以下按"一步一图"方式，从启动到完成一次完整搜索和报告导出，逐一演示每个操作步骤。

#### 5.3.1 启动Web界面

**操作步骤：** 在项目根目录下打开终端，执行以下命令：
...
随后系统会自动在默认浏览器中打开 IntelNexus Web 界面。若未自动打开，手动在浏览器地址栏输入 `http://localhost:8501` 访问。

![图2：Web界面主页面](./images/01_homepage.png)

**Table 2 Web界面主页面全貌**

进入主界面后（如 Table 2 所示），可以看到整个页面分为两大区域：左侧灰色背景的**侧边栏（Sidebar）**和右侧米白色背景的**主面板（Main Panel）**。
...
| 区域 | 位置 | 包含内容 |
|------|------|----------|
| **侧边栏（Sidebar）** | 页面左侧 | 标题"IntelNexus"、搜索模式选择、暗网设置（选暗网模式时）、AI模型选择、线程数设置、语言切换、自定义模型管理、下载格式选择、数据来源说明 |
| **主面板标题区** | 页面右上方 | 大标题"IntelNexus"、副标题"多源网络情报分析平台" |
| **搜索表单区** | 主面板中部 | 搜索输入框（占位文字"输入搜索内容..."）和蓝色"搜索"按钮 |
| **结果展示区** | 搜索表单下方 | 优化查询卡片、搜索结果统计、AI情报报告、搜索结果详情列表 |

#### 5.3.3 侧边栏配置操作

在开始搜索前，需要先在侧边栏完成基本配置。以下逐项说明各配置项的用途和操作方式。

**（1）搜索模式选择**

**操作位置：** 侧边栏顶部"搜索模式"区域

**操作步骤：** 在四个单选按钮中点击选择一个搜索模式：

- **全部来源**（默认选中）：同时搜索网页、新闻和暗网三个数据源
- **网页搜索**：仅通过Bing、DuckDuckGo等搜索引擎搜索网页
- **新闻资讯**：仅搜索Google News、Bing News、Reuters等新闻源
- **暗网搜索**：搜索暗网公开索引（Ahmia）和自定义暗网站点

![图3：侧边栏搜索模式](./images/02_sidebar.png)

**Table 3 侧边栏搜索模式选择**

**预期结果：** 选中的模式按钮高亮显示。若选择"暗网搜索"，侧边栏会额外展开暗网设置面板（详见 5.4 节）。
...
**操作位置：** 侧边栏"设置"区域 → "线程数"滑块

**操作步骤：** 拖动滑块设置并发搜索线程数，范围1-16，默认值为5。

![图5：线程数设置](./images/05_threads.png)
...
**操作位置：** 侧边栏"设置"区域 → "语言"下拉框

**操作步骤：** 从下拉框选择"中文"或"English"，界面文字即时切换。

![图6：语言切换](./images/15_language.png)
...
**操作步骤：** 从 Markdown、PDF、Word、Excel 四种格式中选择一种，此选择将在搜索完成后影响报告的导出格式。

#### 5.3.4 完整搜索操作流程（核心工作流）

以下以搜索"人工智能发展趋势"为例，演示从输入到获取报告的完整操作过程。
...
**操作步骤：** 在输入框中键入查询内容（如"人工智能发展趋势"），输入框占位文字为"输入搜索内容..."。

![图7：搜索输入框](./images/03_search_input.png)

**Table 7 搜索内容输入**

**注意事项：**
- 支持中文和英文输入
- 系统会自动进行拼写纠错和跨语言扩展（中文查询会同时搜索英文相关内容，反之亦然）

---

**▶ 第2步：点击"搜索"按钮**

**操作位置：** 搜索输入框右侧的蓝色"搜索"按钮

**操作步骤：** 确认搜索模式和参数设置无误后，点击"搜索"按钮。

**预期结果：** 系统开始执行搜索流程，页面显示第一个加载动画（spinner），状态文字显示"加载中..."。

![图8：搜索执行中](./images/08_searching.png)
...
**预期结果：** 系统自动完成拼写纠错和多语言查询扩展后，主面板中显示"优化后的查询"信息卡片，包含：

- **原始查询**：用户输入的原关键词
- **多语言查询**：蓝色高亮显示的扩展查询词（如中文查询会附加对应的英文翻译词，用 `|` 分隔）

此卡片让用户了解系统实际使用的搜索关键词范围。

---

**▶ 第4步：多源搜索阶段**

**操作自动进行**，状态文字变为"搜索中..."。系统同时调用网页搜索、新闻搜索（以及暗网搜索，如已选择）模块，并行从多个数据源获取结果。

**预期结果：** 搜索完成后，主面板显示"搜索结果统计"卡片，包含：

- **结果总数**：以醒目大号数字展示（如 "45 条结果"）
- **数据来源分布**：列出各来源返回的结果数量（如 "Bing: 15 | DuckDuckGo: 10 | GoogleNews: 8 | ..."）

![图9：搜索结果统计](./images/09_results.png)
...
系统对搜索返回的每条结果并发抓取其网页全文内容（每篇截取前2000字符），为后续AI分析提供详实的素材基础。
...
主面板显示标题"情报报告"，其下方AI生成的Markdown格式报告内容以**流式输出**方式逐段呈现。

![图10：报告生成中（流式输出）](./images/10_generating.png)
...
报告生成完毕后，状态变为绿色"完成"提示。用户可以滚动页面阅读完整的AI情报分析报告。

![图11：最终报告展示](./images/11_report.png)
...
在报告下方，系统以按来源分组的折叠面板形式展示所有搜索结果的详细列表。

**操作步骤：** 点击各来源分组面板（如 "📌 Bing (15条)"）展开查看其中的搜索结果。

![图12：搜索结果详情列表](./images/13_details.png)

**Table 12 按来源分组的搜索结果详情**
...
**分页导航：** 搜索结果每页显示40条，底部提供"上一页"、"下一页"按钮和页码指示（如 "1/2"）进行翻页浏览。

![图13：分页导航](./images/14_pagination.png.png)
...
**操作步骤：** 从四种格式中选择一种：
- **Markdown**：轻量纯文本，适合技术文档和二次编辑
- **PDF**：专业便携文档，适合正式报告和存档
- **Word**：可编辑的DOCX格式，适合协作编辑
- **Excel**：结构化表格，适合数据分析
...
![图14：下载按钮与格式选择](./images/12_download1.png)

**Table 14 报告下载区域**

**预期结果：** 系统根据所选格式生成导出文件，并在原位置显示具体格式的下载按钮（如"PDF已准备"、"Word已准备"等）。

![图15：格式下载按钮就绪](./images/12_download2.png)

**Table 15 各格式下载按钮就绪状态**

**▶ 第3步：点击格式下载按钮保存文件**

点击对应格式的下载按钮，浏览器自动将报告文件下载到本机默认下载目录。文件名格式为 `report_YYYY-MM-DD_HH-MM-SS.{扩展名}`。
...
**预期结果：** 侧边栏自动展开"🧅 暗网设置"面板，显示以下配置项：

![图16：暗网设置界面](./images/06_darkweb.png)

**Table 16 暗网搜索设置面板**

#### 5.4.2 Tor代理检测与配置

**操作位置：** 暗网设置面板 → Tor端口输入框 和 "检测Tor"按钮

**操作步骤：**

1. **设置Tor端口**：在"Tor端口"输入框中填写Tor代理端口号（默认为9150，Tor浏览器默认SOCKS端口）
2. **启动Tor浏览器**：确保本机已安装并运行Tor浏览器
3. **点击"检测Tor"按钮**：系统检测本地Tor SOCKS5代理是否可用

![图17：Tor状态检测](./images/07_tor_status.png)

**Table 17 Tor运行状态检测结果**

**预期结果：**
- **绿色提示"🟢 已运行"**：表示Tor代理可用，可启用高级模式
- **红色提示"🔴 未运行"**：表示Tor代理不可用，请检查Tor浏览器是否启动

#### 5.4.3 高级模式（Tor代理搜索）

**操作位置：** 暗网设置面板 → "高级模式"复选框

**操作步骤：** 勾选"高级模式"复选框，启用基于Tor代理的暗网站点搜索。

**前提条件：** Tor状态显示为"已运行"（绿色）。若Tor未运行，系统会显示黄色警告提示，此时将自动回退到默认模式（仅使用Ahmia公开索引）。
...
**操作位置：** 暗网设置面板 → "Breached论坛"区域

**操作步骤：**

1. 点击蓝色链接"没有账号？点击注册"打开Breached论坛注册页面（需要通过Tor浏览器访问 `.onion` 地址）
2. 在"用户名"和"密码"输入框中分别填入您的Breached论坛账号信息
3. 两个字段均填写后，系统自动保存并显示绿色"已保存"提示

#### 5.4.5 添加自定义暗网站点

**操作位置：** 暗网设置面板 → "自定义暗网站点"区域

**操作步骤：**

1. **填写站点信息**：
   - 站点名称：如 "My Onion Site"
   - 站点URL：含搜索参数的URL模板，如 `http://xxx.onion/search?q=`
2. **设置认证（可选）**：若站点需要HTTP Basic认证，勾选"需要认证"并填写用户名和密码
3. **点击"添加站点"按钮**：站点配置保存并显示在下方"已添加的站点"列表中

**管理已添加站点：** 每个已添加站点右侧有删除按钮（🗑️），点击即可移除该站点。

#### 5.4.6 执行暗网搜索

暗网配置完成后，搜索操作流程与常规搜索完全一致（参考 5.3.4 节）。区别在于：
- 系统同时从Ahmia公开索引、高级模式源（如已启用Tor）和自定义站点获取结果
- 暗网站点的内容抓取会通过Tor SOCKS5代理进行
...
**操作位置：** 侧边栏底部 → "添加自定义模型"可折叠面板

**操作步骤：** 点击"添加自定义模型"标题展开配置面板。

#### 5.5.2 添加新模型

**操作步骤：**

1. **输入模型名称**：为该模型起一个显示名称（如 "我的GPT-4o"）
2. **选择模型类型**：从14种类型中选择（OpenAI、Anthropic、Google、DeepSeek、Ollama、通义千问、智谱AI、百度文心一言、讯飞星火等）
3. **填写配置参数**（根据所选类型动态显示不同字段）：
   - **OpenAI/云端类型**：需要填写Base URL、API密钥、模型ID
   - **Anthropic**：需要填写API密钥、模型ID
   - **Ollama**：仅需填写Base URL（默认 `http://127.0.0.1:11434`）和模型名称
4. **点击"添加模型"按钮**：配置验证通过后，显示绿色"模型已添加"提示

**预期结果：** 新添加的模型会出现在侧边栏"AI模型"下拉菜单中供选择。

#### 5.5.3 管理已添加模型

**操作位置：** 侧边栏 → "已添加的模型"可折叠面板

**操作步骤：**
- **查看**：展开面板查看所有自定义模型名称
- **删除**：点击模型右侧的"删除"按钮移除该模型

---

### 5.6 操作流程总结

下表汇总 IntelNexus 完整的操作流程及各阶段说明：

| 阶段 | 操作方式 | 系统动作 | 用户可见反馈 |
|------|---------|---------|-------------|
| 1. 启动 | 执行命令 `python main.py ui` | 启动Streamlit Web服务 | 浏览器打开主界面（Table 2） |
| 2. 配置 | 侧边栏设置参数 | 保存设置到会话状态 | 各控件实时响应 |
| 3. 输入 | 输入查询并点击"搜索" | 触发搜索流程 | 按钮按下动画 |
| 4. 优化 | 自动执行 | 拼写纠错 + 多语言扩展 | "优化后的查询"卡片 |
| 5. 搜索 | 自动执行 | 并行调用多搜索引擎 | spinner "搜索中..." + 结果统计卡片 |
| 6. 抓取 | 自动执行 | 并发抓取网页全文 | spinner "抓取内容..." |
| 7. 分析 | 自动执行 | LLM流式生成报告 | 报告内容逐段显示 |
| 8. 查看 | 用户滚动阅读 | — | 完整结构化报告 |
| 9. 导出 | 用户选择格式并点击下载 | 生成对应格式文件 | 浏览器下载文件 |
| 10. 详情 | 用户展开分组面板 | — | 逐条搜索结果 |
...
本附录汇总了IntelNexus软件说明书中所有界面截图的编号、内容和位置，便于审阅者快速查找。

| Table编号 | 截图内容 | 所在章节 | 图片文件 |
|-----------|----------|----------|----------|
| Table 1 | CLI帮助命令输出 | 5.2.2 查看CLI帮助信息 | `01_cli_help.png` |
| Table 2 | Web界面主页面全貌（含左侧功能栏、标题、侧边栏） | 5.3.1 启动Web界面 | `01_homepage.png` |
| Table 3 | 侧边栏搜索模式选择（四个搜索模式全部可见） | 5.3.3 侧边栏配置操作(1) | `02_sidebar.png` |
| Table 4 | AI模型下拉选择菜单展开状态 | 5.3.3 侧边栏配置操作(2) | `04_model_select.png` |
| Table 5 | 线程数滑块设置控件 | 5.3.3 侧边栏配置操作(3) | `05_threads.png` |
| Table 6 | 中英文语言切换下拉框 | 5.3.3 侧边栏配置操作(4) | `15_language.png` |
| Table 7 | 搜索输入框与搜索按钮 | 5.3.4 第1步：输入搜索内容 | `03_search_input.png` |
| Table 8 | 搜索执行中的加载状态（spinner动画） | 5.3.4 第2步：点击搜索按钮 | `08_searching.png` |
| Table 9 | 搜索结果数量与各数据来源分布统计 | 5.3.4 第4步：多源搜索阶段 | `09_results.png` |
| Table 10 | AI报告流式生成过程（内容逐段显示） | 5.3.4 第6步：AI报告生成阶段 | `10_generating.png` |
| Table 11 | AI生成的完整结构化情报报告 | 5.3.4 第7步：查看完整报告 | `11_report.png` |
| Table 12 | 按来源分组的搜索结果详情列表 | 5.3.4 第8步：查看搜索结果详情 | `13_details.png` |
| Table 13 | 搜索结果分页导航控件 | 5.3.4 第8步：分页导航 | `14_pagination.png.png` |
| Table 14 | 报告下载区域（含"下载"按钮） | 5.3.5 第2步：点击下载按钮 | `12_download1.png` |
| Table 15 | 各格式下载按钮就绪状态 | 5.3.5 第2步：格式下载按钮 | `12_download2.png` |
| Table 16 | 暗网搜索设置面板（含Tor端口、检测按钮、高级模式、Breached论坛配置） | 5.4.1 启用暗网搜索 | `06_darkweb.png` |
| Table 17 | Tor运行状态检测结果（绿色"已运行"或红色"未运行"） | 5.4.2 Tor代理检测 | `07_tor_status.png` |

**截图说明：**
- 以上所有截图均为 IntelNexus V1.0 软件真实运行界面截图
- 截图展示了完整的运行界面，包括左侧功能栏（侧边栏）、顶部标题及主内容区域
- 操作类截图按"一步一图"原则，逐一对应第五章各操作步骤
- 所有图片文件存放于 `docs/images/` 文件夹内
